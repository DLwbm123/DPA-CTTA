...
7 域均为 32 legacy + 32 extension = 64 组，总计 448 组；7168 新评分、4480 正式 online，加 12 smoke 为 4492。扩展合格候选数依域次序为 Fundus 368/618/768/69，Polyp 580/158/968。所有选中资产完成新字节身份及编码/几何检查，6 个旧代理摘要匹配封存值；不是旧元数据校验的重命名。

准备阶段首次登记在读取 M4 包装结构时报 KeyError: tasks；改为读取 tasks 成员后成功。该异常发生在选中资产字节验证与 GPU 运行之前；真实 online/outer/inner 均为 0，私有准备失败记录保留。CPU 初版测试的两个夹具断言（FixedProxy 克隆语义及观察器安装后重新 load_state_dict）亦已纠正，没有放宽数值容差。
