...
def oversized(f,code=0):
    out=f.output(200000);p=f.process(code)
    # Inject only at the first exit observation, after process.json is written.
    # A pre-existing overcap tree would correctly make process.json the first failure.
    def poll():
        (out.path/'worker.log').write_bytes(b'x'*220000);return code
    p.poll=poll
    expect(ValueError if code==0 else RuntimeError,lambda:f.supervise(out,cap=200000,start=lambda:p),
