...
with patch('torch.cuda._lazy_init',side_effect=AssertionError('CPU witness: GPU forbidden')),patch.object(execution,'worker',FunctionType(namespace['worker'].__code__,execution.__dict__)):
...
assert len(result.errors)==1 and len(result.failures)==1
assert 'missing_gpu_process' in result.errors[0][1]
assert 'Called 0 times' in result.failures[0][1]
