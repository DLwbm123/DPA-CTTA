...
try:
 for stage in ['run','recompute']:
  with (out/(stage+'.log')).open('x') as log:
   rc=subprocess.call([sys.executable,str(code/'scripts/run_b2_interval.py'),stage,'--receipt',str(args.receipt.resolve())],cwd=code,env=env,stdout=log,stderr=subprocess.STDOUT)
  result['stages'][stage]=rc
  if rc:raise RuntimeError(stage+' exited '+str(rc))
 result.update(status='B2_RUN_RECOMPUTE_REPORT_COMPLETE',exit_code=0)
except Exception as error:result.update(status='B2_PARTIAL',reason=str(error),exit_code=1)
finally:
