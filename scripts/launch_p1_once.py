...
try:
 for stage in ['run','recompute']:
  with (out/(stage+'.log')).open('x') as log:
   rc=subprocess.call([sys.executable,str(code/'scripts/run_p1_no_dd_current_image.py'),stage,'--receipt',str(args.receipt.resolve())],cwd=code,env=env,stdout=log,stderr=subprocess.STDOUT)
  result['stages'][stage]=rc
  if rc:raise RuntimeError(stage+' exited '+str(rc))
 result.update(status='P1_RUN_RECOMPUTE_REPORT_COMPLETE',exit_code=0)
except Exception as error:result.update(status='P1_PARTIAL',reason=str(error),exit_code=1)
finally:
