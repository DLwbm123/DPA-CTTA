...
def prepare_tensors(segmenter,data):
    ...
    for group in 'ABC':
        for static in (False,True):
            ...
            outputs[group+('_STATIC' if static else '_FULL')]=dict(projection_audit=projections,weights=copy.deepcopy(m.state_dict()),validation=trainer.validate(oracles['val']),fit_steps=trainer.fit_steps,cal_steps=trainer.cal_steps,binding=m.digest())
    return dict(models=outputs,oracle_query={f:evaluate_oracles(segmenter,data,o) for f,o in oracles.items()},basis_audit=baudit,cost=dict(COUNTS-before))

def inference_from_tensors(segmenter,group,static,basis,weights,expected_digest,ablation=None):
    ...
    if m.digest()!=expected_digest:raise ValueError('training/inference asset binding')
    m.freeze();return OnlineHost(segmenter,m,ablation=ablation)
