"""GT and scalar metrics enter only after all model, Adam and augmentation RNG commits."""
import time
...
def score(payload,mask):
    try:
        return {name:evaluate(payload[name+'_logits'].sigmoid(),mask,'fundus') for name in ('pre','post')}|{'q':evaluate(payload['q'],mask,'fundus')}
