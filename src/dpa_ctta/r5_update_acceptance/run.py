"""No arguments: metadata only. Real scopes require a new exact R5 authorization."""
...
def main():
    ...
    if os.environ.get('RUN_MODE'):
        ...
        return worker()
    p=argparse.ArgumentParser();p.add_argument('--run',action='store_true');p.add_argument('--registration');p.add_argument('--out');p.add_argument('--assets');p.add_argument('--authorization',default=str(ROOT/'configs/r5_execution.defaults.json'))
    args=p.parse_args()
    if args.run:
        auth=json.loads(Path(args.authorization).read_text())
        if auth.get('enabled') is not True:raise PermissionError('R5 disabled; R4 waiver is not R5 permission')
